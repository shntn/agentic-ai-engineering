...
- `CLAUDE.ja-openrouter.md`は開発時の一時的なドキュメントであり、ソースコードコメントから
  参照しない（「詳細は〜参照」のような自己参照コメントを書かない。理由や対策の説明は
  コメント内で完結させる）
...
- `_call_llm` に `timeout_ms` パラメータを追加し、`chat.send(..., timeout_ms=...)` として
  呼び出しごとに渡す（クライアント初期化時ではなく、Web検索ツールを使う呼び出しにのみ
  明示的に設定する。それ以外の呼び出しはデフォルトのタイムアウトのままでよい）
- `REQUEST_TIMEOUT_MS = 120_000` のような定数を用意し、Web検索を使うステップの
  呼び出しに渡す（最低120秒を目安に）

### 日本語データを扱うモジュールの移植時の注意点

日本語対応が必要なモジュール（メモリ、RAG等）は、ロジックを日本語向けに修正する必要がある。
「動いているように見えるが精度が低い」だけで例外は出ないため、必ず実測で確認すること。

- **キーワード検索**: `str.split()`（スペース区切り）は日本語では機能しない。
  janomeなどの形態素解析器で分かち書き（`tokenize_japanese`）してから比較する。
  BM25のような統計的キーワード検索も同様（英語トークナイザーの代わりに、
  分かち書きしたトークンを直接渡す）
- **ベクトル埋め込み**: 英語中心の埋め込みモデル（all-MiniLM-L6-v2等）は日本語の
  意味的類似度を正しく捉えられない。`intfloat/multilingual-e5-small`等の
  多言語対応モデルに切り替える
  - E5系モデルはquery/passageで異なる接頭辞を要求する。ChromaDBの自動埋め込み
    では接頭辞を使い分けられないため、自前でembedding計算してから
    `embeddings`/`query_embeddings`として明示的に渡す
- **リランカー**: 多言語対応と謳われたモデル（例: FlashRankのms-marco-MultiBERT-L-12）でも
  実測で日本語の精度が低いことがある。採用前に必ず日本語での精度を確認し、
  精度が出ない場合は他のモデル（例: sentence-transformersのCrossEncoder、
  mmarco-mMiniLMv2-L12-H384-v1）を試す
- **チャンク分割**: セパレーターに日本語の句読点（「。」等）を追加する
- **JSON保存**: 日本語を含むデータを保存する際は `ensure_ascii=False` を指定する
  （指定しないと`\uXXXX`エスケープになり可読性が落ちる）

### RAG設計上の注意点

- **検索クエリの設計**: 全文書に頻出する固有名詞を含む質問は、埋め込みベクトルが
  その固有名詞に引っ張られ、正解チャンクが検索結果の上位から漏れることがある
  （日本語では特に、助詞が付いて主語として前面に出やすいため起こりやすい。
  例:「TechFlowは」）。デモ用の質問等は実測で確認し、必要に応じて言い回しを調整する

### プロンプトインジェクション等のテストケースについて

- 攻撃プロンプト自体（テストデータとして使われる文字列）は、原文の英語のまま維持する
  （日本語訳すると、攻撃パターンとしての妥当性が変わる可能性があるため）
- 内容の理解のため、直前にコメントで日本語訳を添える
- 周辺の説明・レポート出力等は通常通り日本語化する
